...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            key_layer = self.transpose_for_scores(self.key(hidden_states))
            value_layer = self.transpose_for_scores(self.value(hidden_states))
        # self.cache["hidden_states"] = value_layer
        ...
        # print(f"key_layer.contiguous = {key_layer.is_contiguous()}")
        # print(f"query_layer.contiguous = {query_layer.is_contiguous()}")
        # dim1, dim2, dim3, dim4 = key_layer.shape
        # attention_scores = torch.matmul(query_layer, key_layer.reshape((dim1, dim2, dim4, dim3)))
        ...
        # add by yutian, for distill hook
        if output_attentions:
            outputs = outputs + (query_layer, key_layer, value_layer)
        return outputs
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        # print(
        #     "ori layer 11 val ",
        #     self.layer[-1].attention.self.cache["value_state"].size(),
        # )
        # print(
        #     "ori layer 11 val ",
        #     self.layer[-1].attention.self.cache["value_state"][:2, :3, :3, :3],
        # )
        # print(
        #     "ori layer 11 attn ",
        #     self.layer[-1].attention.self.cache["attention_scores"].size(),
        # )
        # print(
        #     "ori layer 11 attn ",
        #     self.layer[-1].attention.self.cache["attention_scores"][:2, :3, :3, :3],
        # )
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True, dtr=False):
    ...
    def forward(
        ...
        if self.dtr:
            # print("\n\n\ndtr in BertModel()\n\n\n")
            input_tensors = [input_ids, attention_mask, token_type_ids, position_ids, head_mask, inputs_embeds, \
                             encoder_hidden_states, encoder_attention_mask, past_key_values, \
                             extended_attention_mask, encoder_extended_attention_mask, past_key_values_length]
            for t in input_tensors:
                if t != None and type(t) in (torch.Tensor, torch.FloatTensor):
                    t.detach().checkpoint()
...
class BertLMHeadModel(BertPreTrainedModel):
    ...
    def prepare_inputs_for_generation(
        self, input_ids, past=None, attention_mask=None, use_cache=True, **model_kwargs
    ):
...
class BertForMaskedLM(BertPreTrainedModel): # mark
    ...
    def __init__(self, config, dtr=False):
        super().__init__(config)
        ...
        self.bert = BertModel(config, add_pooling_layer=False, dtr=dtr)
        self.cls = BertOnlyMLMHead(config)
    ...
    def forward(
        ...
        inp_params = locals()
        # print("\n\n\n")
        # print([k + " (" + str(type(v) == torch.Tensor) + "), " for k,v in inp_params.items()])
        # print("\n\n\n")
    ...
    def forward(
        ...
        **kwargs,
        ...
        if labels is not None:
            loss_fct = CrossEntropyLoss()
            next_sentence_loss = loss_fct(
                seq_relationship_scores.reshape(-1, 2), labels.reshape(-1)
            )
